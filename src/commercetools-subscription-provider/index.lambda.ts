...
const processEvent: CdkCustomResourceHandler = async (event: CdkCustomResourceEvent, context: LambdaContext) => {
  ...
  switch (event.RequestType) {
    ...
    case "Delete": {
      ...
      const currentSubscription = await apiRoot.withProjectKey({projectKey: props.projectKey}).subscriptions().withId({ID: id}).get().execute()
      if (currentSubscription.body.version != version) {
        return Promise.resolve(<CdkCustomResourceResponse>{
          PhysicalResourceId: event.PhysicalResourceId
        })
      }
      const ret = await apiRoot.withProjectKey({projectKey: props.projectKey}).subscriptions().withId({ID: id}).delete({queryArgs: {version}}).execute()
      return Promise.resolve(<CdkCustomResourceResponse>{
        PhysicalResourceId: `${ret.body.id}:${ret.body.version}`,
        Data: {
          ...{
